...
class Driver implements opslang.Driver {
  ...
  variables: Map<string, opslang.Value> = new Map();
  ...
  async sendCommand(
    ...
    const commandLine: CommandLine = {
      ...
      parameters: params.map((arg): ParameterValue => {
        if (arg.kind === "integer") {
          return {
            type: "integer",
...
export const CommandView: React.FC = () => {
  ...
  const handleEditorDidMount = useCallback(
    (editor: monaco.editor.IStandaloneCodeEditor, monacoInstance: Monaco) => {
      ...
      const processLine = async (firstLine: number): Promise<boolean> => {
        ...
        const result = await executeLine(line, lineno == firstLine);
        if (!result.success) {
          decoration.clear();
      ...
      editor.addCommand(
        ...
        async () => {
          ...
          while (await processLine(firstLine)) {
            /* do nothing */
          }
        }
      );
