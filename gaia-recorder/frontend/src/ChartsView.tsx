...
type SatelliteSchema = {
  // Old format (satconfig.json)
  telemetryChannels?: Record<string, unknown>;
  telemetryComponents?: Record<
    string,
    {
      telemetries?: Record<string, { fields?: Array<{ name: string; valueType: string }> }>;
    }
  >;
  // New format (tlmcmddb.json)
  components?: Array<{
    name: string;
    tlm?: {
      telemetries?: Array<{
...
const ChartPanel: React.FC<ChartPanelProps> = ({
  ...
  const buildAxesAndScales = useCallback(() => {
    ...
    for (const s of categoricalSeries) {
      ...
      scales[`y_${s.id}`] = {
        auto: false,
        range: buildCategoricalRange(values) as [number, number],
      };
    }
    return { axes, scales };
  }, [series, valueMapsRef]);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            text="Reset Zoom"
            onClick={() => {
              plotRef.current?.setScale("x", { min: undefined as any, max: undefined as any });
              onResetZoom();
            }}
          />
        </div>
...
export const ChartsView: React.FC = () => {
  ...
  const [satelliteSchema, setSatelliteSchema] = useState<SatelliteSchema | null>(null);
  ...
  const tmivNames = useMemo(() => {
    ...
    // New format (tlmcmddb.json with components array)
    if (satelliteSchema.components) {
      // Default channel names for tlmcmddb format
      const channelNames = ["RT", "DR"];
      for (const component of satelliteSchema.components) {
        if (component.tlm?.telemetries) {
          for (const telemetry of component.tlm.telemetries) {
            for (const channelName of channelNames) {
              items.push(`${channelName}.${component.name}.${telemetry.name}`);
            }
          }
        }
      }
    }
    // Old format (satconfig.json with telemetryComponents object)
    else if (satelliteSchema.telemetryComponents) {
  ...
  const fieldNames = useMemo(() => {
    ...
    // New format (tlmcmddb.json)
    if (satelliteSchema.components) {
      const component = satelliteSchema.components.find((c) => c.name === componentName);
      const telemetry = component?.tlm?.telemetries?.find((t) => t.name === telemetryName);
      if (telemetry?.entries) {
        // Extract field names from entries
        for (const entry of telemetry.entries) {
          if (entry.type === "FIELD" && entry.name) {
            names.push(entry.name);
          } else if (entry.type === "FIELD_GROUP" && entry.sub_entries) {
            for (const subEntry of entry.sub_entries) {
              if (subEntry.type === "FIELD" && subEntry.name) {
    ...
    // Old format (satconfig.json)
      ...
      if (telemetry) {
        names = (telemetry.fields || []).map((f: { name: string }) => f.name);
      }
    }
